package com.example.themovie.feature.home.pager.now_playing
...
class NowPlayingFragment : ReactiveView<State, Intent>(), MovieAdapter.Callback {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        bind(NowPlayingViewModel::class, sharedWithActivity = true)
        initMovieAdapter()
        ...
        lifecycleScope.launch {
            SortingOptionsByRecommendation.sortingOptionsNowPlaying.collect {
                sortingOption = it.name
                intent(Intent.SortMovies(it.name, recommendationType))
            }
        }
    ...
    override fun onAddToFavoritesClicked(movieId: Int) {
        intent(Intent.OnAddToFavoritesClicked(movieId, recommendationType))
    }

    override fun onRemoveFromFavoritesClicked(movieId: Int) {
        intent(Intent.OnRemoveFromFavoritesClicked(movieId, recommendationType))
    }
    ...
    override fun render(state: State) {
        with(state) {
            ...
            movieAdapterPosition.get()?.let {
                if (it != 0) {
                    viewBinding.moviesRv.smoothScrollToPosition(it)
